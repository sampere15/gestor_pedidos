...
@section('content')
	...
	<div class="row col-md-12 cold-sm-12">
		<form class="form-horizontal" id="formularioEditarPedido" method="POST" action="{{ route('pedidos.actualizar', $pedido) }}">
			@csrf
			@method('PUT')
			{{-- Con este parámetro indicamos si el pedido se va a guardar o tramitar/actualizar --}}
			<input type="hidden" name="tramitarPedido" id="tramitarPedido" value="null">
			...
			<div class="form-group">
			    ...
			    <div class="col-md-2 col-sm-12">
				    <input required autocomplete="off" type="text" class="form-control" name="proveedorNombre" id="btnBuscarProveedor" value="{{ $pedido->proveedor->nombre }}">
			    </div>
			    ...
			    <div class="col-md-2 col-sm-12">
					<select required class="form-control" id="departamentoSelect" name="departamentoSelect" onchange="ActualizarCampos()">
						@foreach ($departamentos as $departamento)
							<option value="{{ $departamento->id }}" {{ $pedido->departamento_id == $departamento->id ? "selected" : "" }}>{{ $departamento->nombre }}</option>
						@endforeach
				    </select>
				</div>
				...
				<div class="col-md-2 col-sm-12">
					<select class="form-control" id="campoSelect" name="campoSelect" required onchange="CampoSeleccionado()">
						@foreach (Auth::user()->camposConPermisoSegunDepartamento($pedido->departamento_id) as $campo)
							<option value="{{ $campo->id }}" {{ $pedido->campo_id == $campo->id ? "selected" : "" }}>{{ $campo->nombre }}</option>
						@endforeach
					</select>
				</div>
			    ...
			    <div class="col-md-2 col-sm-12">
				    <select class="form-control" id="sociedadSelect" name="sociedadSelect">
						@foreach ($pedido->campo->sociedades as $sociedad)
							<option value="{{ $sociedad->id }}" {{ $pedido->sociedad_id == $sociedad->id ? "selected" : "" }}>{{ $sociedad->nombre }}</option>
						@endforeach
				    </select>
				</div>
				...
				<div class="col-md-5 col-sm-10 col-xs-10">
					<select class="form-control" id="direccionSelect" name="direccionSelect" value="{{ old('direccionSelect') }}" onchange="SelectDireccionCambiado()">
						@foreach ($pedido->campo->direcciones as $direccion)
							<option value="{{ $direccion->id }}" {{ $direccion->id == $pedido->direccion_id ? "selected" : "" }}>{{ $direccion->nombre }}</option>
						@endforeach
					</select>
				</div>
							...
							<tbody>
								@foreach ($pedido->lineasPedido as $lineaPedido)
									<tr id="lineaPedido{{ $lineaPedido->numero_linea }}">
										...
										<td><input class="form-control" type="text" name="inputDescripcion{{ $lineaPedido->numero_linea }}" value="{{ $lineaPedido->descripcion }}" required></td>
										<td><input class="form-control" onchange="ActualizarTotalLineaPedido(this)" numero_linea="{{ $lineaPedido->numero_linea }}" type="text" name="inputUnidades{{ $lineaPedido->numero_linea }}" id="inputUnidades{{ $lineaPedido->numero_linea }}" value="{{ $lineaPedido->unidades }}" required></td>
										<td>
										...
										<td><input class="form-control" onchange="ActualizarTotalLineaPedido(this)" numero_linea="{{ $lineaPedido->numero_linea }}" type="text" name="inputPrecio{{ $lineaPedido->numero_linea }}" id="inputPrecio{{ $lineaPedido->numero_linea }}" onkeydown="if (event.keyCode == 9) InsertarFilaTabla()" value="{{ $lineaPedido->precio }}" onkeyup="CheckDecimales(this)" required></td>
...
@push('footscripts')
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() 
        {
			...
			ActualizarDatosModalDireccion({!! $pedido->direccion !!})
		...
		{
			//	Recuperamos el id del campo sobre el que hemos pulsado
			campo_seleccionado_id = document.getElementById("campoSelect").value;
			console.log(campo_seleccionado_id);
			//	Recuperamos la dirección para ese campo
			ActualizarDirecciones();
			//	Recuperamos las sociedades que gestionan este campo
			ActualizarDatosSocidades();
		}
        ...
        function InsertarFilaTabla()
        {
        	...
        	nuevaLinea.insertCell(5).innerHTML = `<input id="inputPrecio${lineasPedido}" class="form-control" type="text" onchange="ActualizarTotalLineaPedido(this)" name="inputPrecio${lineasPedido}" numero_linea="${lineasPedido}" onkeydown="if (event.keyCode == 9) InsertarFilaTabla()" required onkeyup="CheckDecimales(this)">`;
