...
@section('content')
	<div class="row">
		<div class="col-md-6 col-sm-6">
			<h2>Edición del pedido nº {{ $pedido->id }}</h2>	
		</div>
	</div>

	<div class="row">
		<div class="col-md-3 col-sm-3 pull-right">
			<h3>Creado por: <small>{{ $pedido->usuarioRealizaPedido->nombre . ' ' . $pedido->usuarioRealizaPedido->apellidos }}</small></h3>	
		</div>
	</div>

	<div class="row col-md-12 cold-sm-12">
		<form class="form-horizontal" id="formularioEditarPedido" method="POST" action="{{ route('pedidos.actualizar', $pedido) }}">
			@csrf
			@method('PUT')
			<input type="hidden" name="tramitarPedido" id="tramitarPedido">
			
			<div class="form-group">
				{{-- Proveedor --}}
			    <label for="btnBuscarProveedor" class="col-md-1 col-sm-12 control-label">Proveedor:</label>
			    <div class="col-md-2 col-sm-12">
				    <input type="text" class="form-control" id="btnBuscarProveedor" value="{{ $pedido->proveedor->nombre }}">
			    </div>
				<input type="hidden" id="proveedor_id" name="proveedor_id" value="{{ $pedido->proveedor->id }}">
			    ...
			    <div class="col-md-2 col-sm-12">
				    <select class="form-control" id="departamentoSelect" name="departamentoSelect">
				    	@foreach ($departamentos as $departamento)
				    		<option value="{{ $departamento->id }}" {{ $pedido->departamento->id == $departamento->id ? "selected" : "" }}>{{ $departamento->nombre }}</option>
				    	@endforeach
				    </select>
				</div>
			    ...
			    <div class="col-md-2 col-sm-12">
				    <select class="form-control" id="campoSelect" name="campoSelect" onchange="ActualizarDatos()">
				    	@foreach ($campos as $campo)
				    		{{-- <option @if($campo->nombre == $pedido->campo->nombre) selected="true" @endif value="{{ $campo->id }}">{{ $campo->nombre }}</option> --}}
				    		<option value="{{ $campo->id }}" {{ $campo->id == $pedido->campo->id ? "selected" : "" }}>{{ $campo->nombre }}</option>
				    	@endforeach
				    </select>
				</div>
				
				{{-- Sociedad --}}
			    <label for="socieadSelect" class="col-md-1 col-sm-12 control-label">Sociead:</label>
			    <div class="col-md-2 col-sm-12">
				    <select class="form-control" id="socieadSelect" name="sociedadSelect">
				    	@foreach ($pedido->campo->sociedades as $sociedad)
				    		...
				    		<option value="{{ $sociedad->id }}" {{ $sociedad->id == $pedido->sociedad->id ? "selected" : "" }}>{{ $sociedad->nombre }}</option>
				    	@endforeach
				    </select>
			    </div>
			</div>
			<div class="form-group">
				...
				<div class="col-md-5 col-sm-10 col-xs-10">
					<select class="form-control" id="direccionSelect" name="direccionSelect" onchange="SelectDireccionCambiado()">
						@foreach ($pedido->campo->direcciones as $direccion)
							<option value="{{ $direccion->id }}" {{ $direccion->id == $pedido->direccion->id ? "selected" : "" }}>{{ $direccion->nombre }}</option>
						@endforeach
					</select>
				</div>
										...
										<td>
											<select class="form-control" id="selectCategoria{{ $lineaPedido->numero_linea }}" name="selectCategoria{{ $lineaPedido->numero_linea }}">
												@foreach ($categorias as $categoria)
													<option value="{{ $categoria->id }}" {{ $categoria->id == $lineaPedido->categoria->id ? "selected" : "" }}>{{ $categoria->nombre }}</option>
												@endforeach
											</select>
										</td>
										<td><input class="form-control" type="text" name="inputDescripcion{{ $lineaPedido->numero_linea }}" value="{{ $lineaPedido->descripcion }}"></td>
										<td><input class="form-control" onchange="ActualizarTotalLineaPedido(this)" numero_linea="{{ $lineaPedido->numero_linea }}" type="text" name="inputUnidades{{ $lineaPedido->numero_linea }}" id="inputUnidades{{ $lineaPedido->numero_linea }}" value="{{ $lineaPedido->unidades }}"></td>
										<td>
											<select class="form-control" id="selectFormato{{ $lineaPedido->numero_linea }}" name="selectFormato{{ $lineaPedido->numero_linea }}">
												@foreach ($formatos as $formato)
													<option value="{{ $formato->id }}" {{ $formato->id == $lineaPedido->formato->id ? "selected" : "" }}>{{ $formato->nombre }}</option>
												@endforeach
											</select>
										</td>
										<td><input class="form-control" onchange="ActualizarTotalLineaPedido(this)" numero_linea="{{ $lineaPedido->numero_linea }}" type="text" name="inputPrecio{{ $lineaPedido->numero_linea }}" id="inputPrecio{{ $lineaPedido->numero_linea }}" value="{{ $lineaPedido->precio }}"></td>
										<td><input class="form-control totalLinea" type="readonly" name="inputTotal{{ $lineaPedido->numero_linea }}" id="inputTotal{{ $lineaPedido->numero_linea }}" readonly
											value="{{ $lineaPedido->unidades * $lineaPedido->precio }}"></td>
										<td>
			...
			<div class="form-group">
				<label>
				    <input type="checkbox" name="cbEstadosEspaciales" onclick="ComprobarEstadosEspeciales(this)" {{ $pedido->solicitado_al_proveedor || $pedido->ya_recibido ? "checked" : "" }}> Marcar un estado especial
				</label>
			</div>
			<div class="form-group">
                <label>
                    <input type="radio" id="rbSolicitadoProveedor" name="rbEstadosEspeciales" value="rbSolicitadoProveedor" {{ $pedido->solicitado_al_proveedor ? "checked" : ""}} {{ !$pedido->solicitado_al_proveedor && !$pedido->ya_recibido ? "disabled" : "" }}> Se ha contactado con el proveedor para que marche el pedido
                </label>
            </div>
            <div class="form-group">
				<label>
                    <input type="radio" id="rbMaterialRecibido" name="rbEstadosEspeciales" value="rbMaterialRecibido" {{ $pedido->ya_recibido ? "checked" : ""}} {{ !$pedido->solicitado_al_proveedor && !$pedido->ya_recibido ? "disabled" : "" }}> Ya se ha recibido el material
                </label>
			</div>
			<hr style="border-style: inset; border-width: 1px;">
			<div class="form-group">
			...
			<input type="hidden" id="totalLineasPedidoInput" name="totalLineasPedidoInput" value="{{ count($pedido->lineasPedido) }}">
		</form>
	</div>
...
@push('footscripts')
    ...
    <script type="text/javascript">
    	...
    	var array_direcciones = {!! $pedido->campo->direcciones !!};
    	var direccionPedido = {!! $pedido->direccion !!};	//	Como el pedido ya tiene una dirección, esta será la que vamos a cargar en el modal de dirección
    	var campoSeleccionado_id = {!! $pedido->campo->id !!};	//	Guarda el ID del campo que tenemos seleccionado en el select
    	var tablaLineasPedido = document.getElementById("tablaLineasPedido");	//	Referencia a nuestra tabla de líneas del pedido
    	var lineasPedido = {!! count($pedido->lineasPedido) !!};	//	variable para contabilizar cuantas líneas de pedidos llevamos. Hará de ID de nuestra línea
        ...
        $(document).ready(function() 
        {
            ...
            ActualizarDatosModalDireccion(direccionPedido);
        ...
        $('#btnGuardarYContinuar').click(function()
        {
        	...
        	// $('#formularioEditarPedido').attr('action', rutaGuardarPedido);
        	$('#tramitarPedido').attr('value', false);
        	$('#formularioEditarPedido').submit();
        });

        $('#btnSolicitarPedido').click(function()
        {
        	//	Configuramos el action del formulario para que realice una accion u otra
        	// $('#formularioEditarPedido').attr('action', rutaSolicitarPedido);
        	$('#tramitarPedido').attr('value', true);
        	$('#formularioEditarPedido').submit();
        });

        //	Cuando cambia de campo en el select, tenemos que recuperar los datos de sus sociedades y posibles direcciones
        ...
        function ConfirmarEliminarLineaPedido()
        {
        	...
        	ActualizarTotalPedido();		//	Ya que hemos eliminado una línea, actualizamos el precio
        }
